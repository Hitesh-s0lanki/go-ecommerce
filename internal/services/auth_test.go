...
// Logging out twice is not an error: the caller wanted the session gone.
func TestLogoutIsIdempotent(t *testing.T) {
	svc, _ := newAuthService(t)
	ctx := context.Background()

	reg, err := svc.Register(ctx, registerReq("idem@example.com"))
	if err != nil {
		t.Fatalf("Register: %v", err)
	}
...
func TestCurrentUser(t *testing.T) {
	...
	reg, err := svc.Register(ctx, registerReq("current@example.com"))
	if err != nil {
		t.Fatalf("Register: %v", err)
	}

	user, err := svc.CurrentUser(ctx, reg.User.ID)
	if err != nil {
		t.Fatalf("CurrentUser: %v", err)
	}
	if user.Email != "current@example.com" {
		t.Errorf("Email = %q, want current@example.com", user.Email)
	}

	// A valid token for a deactivated account must stop working.
	if err := db.Model(&models.User{}).Where("id = ?", reg.User.ID).
		Update("is_active", false).Error; err != nil {
		t.Fatalf("deactivate: %v", err)
	}

	if _, err := svc.CurrentUser(ctx, reg.User.ID); !errors.Is(err, services.ErrInvalidCredentials) {
		t.Errorf("err = %v, want ErrInvalidCredentials for a deactivated account", err)
	}
}
